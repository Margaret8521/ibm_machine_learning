...
def get_pie_chart(site_dropdown):
    ...
    if site_dropdown == 'ALL':
        ...
        names='class', 
        title='Total Launches for a Specific Site')
        return fig
...
def get_scatter_plot(site_dropdown, payload_slider):
    ...
    if site_dropdown == 'ALL':
        ...
        scatter = px.scatter(payload_df, x='Payload Mass (kg)', y='class', color='Booster Version Category')
        return scatter
    else:
        # return the outcomes piechart for a selected site
        payload_df = filtered_df[(filtered_df['Payload Mass (kg)']>=payload_slider[0])
        &(spacex_df['Payload Mass (kg)']<=payload_slider[1])]
        scatter = px.scatter(payload_df, x='Payload Mass (kg)', y='class', color='Booster Version Category')
        return scatter
# Run the app
